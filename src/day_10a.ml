...
let get_right_constellation (initial_points: point list): point list =
    let move_points (points: point list): point list =
        List.map (fun {position = {x; y}; v_x; v_y} -> {position = {x = x + v_x; y = y + v_y}; v_x; v_y} ) points in
    let rec loop (points: point list) (volume: int): point list =
        let points' = move_points points in
        let volume' = get_volume_of_enclosing_box points' in
        if volume' >= volume then points else loop points' volume'
    in loop initial_points (get_volume_of_enclosing_box initial_points)


let display_message (points: point list): unit =
    let {x_min; x_max; y_min; y_max} = calc_enclosing_box points in
    let grid = Array.make_matrix (y_max - y_min + 1) (x_max - x_min + 1) "." in
    List.iter (fun {position = {x; y}} -> grid.(y - y_min).(x - x_min) <- "#") points;
    let rows = Array.map (fun row -> row |> Array.to_list |> String.concat "") grid in
